...
@interface LevelNineteen ()

@end

@implementation LevelNineteen
...
/////////
////////    button1 for inc 2 for random 1 / button 2 for inc 1 for random 2
/////////
...
-(IBAction)sendStart:(id)sender{
    ...
    loopNumber2 = 1;
    loopLabel2.text = [[NSString alloc] initWithFormat:@"Loop #: %d", loopNumber2];
    
    //incrementing timer start
    
    MainInt = 0;
    timer = [NSTimer scheduledTimerWithTimeInterval: 0.09 target:self selector:@selector(countup) userInfo:nil repeats:YES];
    
    MainInt2 = 0;
    timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.07 target:self selector:@selector(countup2) userInfo:nil repeats:YES];
...
-(IBAction)sendStop:(id)sender{
    ...
    if(MainInt == ranNumber ){
        ...
        if ([incrementNumberLabel2.text isEqualToString:@"PASS"]){
            ...
            MainInt2 = 0;
            timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.07 target:self selector:@selector(countuplast2) userInfo:nil repeats:YES];
            
            buttonFlipTimer2 = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount2) userInfo:nil repeats:YES];
            
            ranNumber2 = [self getRandomNumberBetweenMin:90 andMax:130];
...
-(IBAction)sendStop2:(id)sender{
    ...
    if(MainInt2 == ranNumber2 ){
        ...
        if ([incrementNumberLabel.text isEqualToString:@"PASS"]){
            ...
            MainInt = 0;
            timer = [NSTimer scheduledTimerWithTimeInterval: 0.09 target:self selector:@selector(countuplast) userInfo:nil repeats:YES];
            
            buttonFlipTimer = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount) userInfo:nil repeats:YES];
            
            ranNumber = [self getRandomNumberBetweenMin:120 andMax:150];
