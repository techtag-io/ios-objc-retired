...
-(IBAction)sendStart:(id)sender{
    ...
    MainInt = 0;
    timer = [NSTimer scheduledTimerWithTimeInterval: 0.15 target:self selector:@selector(countup) userInfo:nil repeats:YES];
    
    MainInt2 = 0;
    timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.25 target:self selector:@selector(countup2) userInfo:nil repeats:YES];
...
-(IBAction)sendStop:(id)sender{
    ...
    if(MainInt == ranNumber ){
        ...
        if ([incrementNumberLabel2.text isEqualToString:@"PASS"]){
            ...
            MainInt2 = 0;
            timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.25 target:self selector:@selector(countuplast2) userInfo:nil repeats:YES];
            
            buttonFlipTimer2 = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount2) userInfo:nil repeats:YES];
            
            ranNumber2 = [self getRandomNumberBetweenMin:30 andMax:45];
...
-(IBAction)sendStop2:(id)sender{
    ...
    if(MainInt2 == ranNumber2 ){
        ...
        if ([incrementNumberLabel.text isEqualToString:@"PASS"]){
            ...
            MainInt = 0;
            timer = [NSTimer scheduledTimerWithTimeInterval: 0.15 target:self selector:@selector(countuplast) userInfo:nil repeats:YES];
            
            buttonFlipTimer = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount) userInfo:nil repeats:YES];
            
            ranNumber = [self getRandomNumberBetweenMin:40 andMax:50];
