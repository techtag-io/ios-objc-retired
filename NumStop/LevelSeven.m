...
//  NumStop
//
//  Created by Travis McGowan on 6/2/12.
...
-(IBAction)sendStart:(id)sender{
    ...
    MainInt2 = 0;
    timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.4 target:self selector:@selector(countup2) userInfo:nil repeats:YES];
...
-(IBAction)sendStop:(id)sender{
    ...
    if(MainInt == ranNumber ){
        ...
        if ([incrementNumberLabel2.text isEqualToString:@"PASS"]){
            ...
            MainInt2 = 0;
            timer2 = [NSTimer scheduledTimerWithTimeInterval: 0.4 target:self selector:@selector(countuplast2) userInfo:nil repeats:YES];
            
            buttonFlipTimer2 = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount2) userInfo:nil repeats:YES];
            
            ranNumber2 = [self getRandomNumberBetweenMin:20 andMax:30];
...
-(IBAction)sendStop2:(id)sender{
    ...
    if(MainInt2 == ranNumber2 ){
        ...
        if ([incrementNumberLabel.text isEqualToString:@"PASS"]){
            ...
            MainInt = 0;
            timer = [NSTimer scheduledTimerWithTimeInterval: 0.2 target:self selector:@selector(countuplast) userInfo:nil repeats:YES];
            
            buttonFlipTimer = [NSTimer scheduledTimerWithTimeInterval: 0.12 target:self selector:@selector(buttonFlipCount) userInfo:nil repeats:YES];
            
            ranNumber = [self getRandomNumberBetweenMin:25 andMax:40];
...
-(void) alertView :(UIAlertView *) alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if ([select isEqualToString:@"Play Again"] || [select isEqualToString:@"Restart"]){ 
        ...
        ranNumber2 = [self getRandomNumberBetweenMin:1 andMax:10];
